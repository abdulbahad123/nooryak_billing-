...
const common = () => {
    ...
    const formatAmountCurrency = (amount) => {
        ...
        var thousandSeparatorType = appSetting.value.currency.thousand_separator == "none" ? "" :
            appSetting.value.currency.thousand_separator == "comma" ? "," :
            appSetting.value.currency.thousand_separator == "space" ? " " :
            appSetting.value.currency.thousand_separator == "comma" ? "," : ".";
        var decimalSeparatorType =
            appSetting.value.currency.decimal_separator === "comma" ? "," :
            appSetting.value.currency.decimal_separator === "dot" ? "." :
            " ";


        var newAmount = parseFloat(Math.abs(amount)).toFixed(2).replace('.', decimalSeparatorType).replace(/\B(?=(\d{3})+(?!\d))/g, thousandSeparatorType);

        if (appSetting.value.currency.remove_decimal_with_zero !== 0 && newAmount.endsWith(decimalSeparatorType + '00')) {
            newAmount = newAmount.slice(0, -decimalSeparatorType.length - 2);
        }

        var space = appSetting.value.currency.space_between_price_and_price_symbol == 1 ? " " : "";

        if (appSetting.value.currency.position == "front") {
            var newAmountString = `${appSetting.value.currency.symbol}${space}${newAmount}`;
        } else {
            var newAmountString = `${newAmount}${space}${appSetting.value.currency.symbol}`;
        }
